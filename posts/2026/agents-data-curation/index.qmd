...
We used [SetFit](https://huggingface.co/docs/setfit/index), a library for training text classifiers from small labelled datasets, with a [149-million-parameter ModernBERT encoder](https://huggingface.co/nomic-ai/modernbert-embed-base). The agent wrote the scripts and ran training and comparisons on Jobs. Long documents were represented by excerpts from their beginning, middle and end. The [trained classifier and prediction helper](https://huggingface.co/davanstrien/finepdfs-edu-purpose-classifier) are available on the Hub.
...
Here, `run_environment` includes the pinned input revision and runtime settings. The [complete inference script](https://huggingface.co/davanstrien/finepdfs-edu-purpose-classifier/blob/main/reproduction/classify.py) also handles saving progress and checking inputs.
